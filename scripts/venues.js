...
inputTag.addEventListener("input", debounce(fetchFunction, 500));
  ...
  if (data.length >= 1) {
    ...
    data.map((el) => {
      ...
      mainDiv.addEventListener("click", () => {
        ...
        let arr = [];
        arr.push(el);
        localStorage.setItem("singleVenueDetails", JSON.stringify(arr[0]));
      });
    });
    ...
    inputTag.addEventListener("keypress", (event) => {
      //console.log("Test");
      if (event.key == "Enter") {
        window.location.href = "allVenues.html";
